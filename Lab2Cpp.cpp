...
int* EnterArr(int* arr, int number_of_array_elements) {
	...
	return arr;
}int SortArr(int* arr, int number_of_array_elements) {
...
int Composition(int* arr, int number_of_array_elements) {
	...
	if (zero_pos1 > number_of_array_elements or zero_pos2 > number_of_array_elements) {
		...
		for (int i = (zero_pos1 + 1); i < zero_pos2; i++)
		{
			composition *= arr[i];
		};
	}
...
int main()
{
	...
	EnterArr(arr, number_of_array_elements);
	cout << "Composition : " << Composition(arr, number_of_array_elements) << endl;
	cout << "Number of sign changes : " << NumberOfSighnChanges(arr, number_of_array_elements) << endl;
	SortArr(arr, number_of_array_elements);
	run_test();
}